...
5. Print one summary line: `profile NAME: N created, M changed, K tasks run, in sync`, or, when nothing needed applying, `profile NAME: already running, brought to front`. `--json` adds `"failed"` and `"skipped"` arrays to the report.
