...
<!-- log-driven-workspace:start — eventlog consumer instructions; reactor assets refreshed with eventlog setup upgrade -->
...
- Read the log: `eventlog view --last 20`, or `jq -c . .context/events.jsonl`.

### Boundaries (everyone)

- Only `eventlog append` writes the log, and only the controller and the reactors run it. Never edit, truncate or `rm` the log; never `rm` a `*.reactor.lock` dir.
- Never `git commit`; the commit reactor does that from a `result`.
- Never message, ping or restart a reactor. If one looks dead, say so; the user authorizes recovery through `drove up --no-focus --yes`.
...
- Worker briefs: `.context/handoffs/`. Reactor settings: `.context/eventlog-setup.toml`; run `eventlog setup preview` then `eventlog setup upgrade` after policy changes.
